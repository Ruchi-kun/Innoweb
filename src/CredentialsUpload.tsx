...
export default function CredentialsUpload() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) await readPdfFile(file);
  };
  ...
  return (
      <div className="min-h-screen w-full relative overflow-x-hidden overflow-y-auto py-12">
        ...
        {/* FOREGROUND CONTENT */}
        {status === 'success' ? (
            <div className="w-full max-w-2xl mx-auto mt-20 flex flex-col items-center font-sans bg-white p-12 rounded-3xl shadow-2xl border border-emerald-100 text-center relative z-10">
            ...
            <div className="w-full max-w-2xl mx-auto mt-12 flex flex-col items-center font-sans relative z-10">
              <h1 className="text-white text-[2.5rem] font-bold mb-3 tracking-tight">
                Welcome to Innoweb
              </h1>
              ...
              <div className="w-full bg-white rounded-2xl shadow-2xl border border-slate-100 p-8 sm:p-10 mb-8 relative overflow-hidden">
                {status === 'analyzing' && (
                ...
                <div className="mb-8 relative">
                  <h2 className="text-[#1E293B] font-medium text-[1.05rem] mb-4">Upload company credentials</h2>
                  <label
                      onDragOver={handleDragOver}
                      onDragLeave={handleDragLeave}
                      onDrop={handleDrop}
                      className={`border-2 border-dashed rounded-xl transition-colors py-12 px-6 flex flex-col items-center justify-center cursor-pointer group relative overflow-hidden ${isDragging ? 'border-[#3B4569] bg-blue-50' : 'border-[#CBD5E1] bg-slate-50/50 hover:bg-slate-50'}`}
                  >
                    <input type="file" className="hidden" onChange={handleFileChange} accept=".pdf" />
                    {fileName ? (
                        <div className="w-14 h-14 bg-emerald-50 rounded-full flex items-center justify-center mb-4 border border-emerald-100 shadow-sm"><svg viewBox="0 0 24 24" fill="none" stroke="#10b981" strokeWidth="2.5" className="w-7 h-7"><polyline points="20 6 9 17 4 12"></polyline></svg></div>
                    ) : (
                        <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center mb-4 border border-slate-100 shadow-sm"><svg viewBox="0 0 24 24" fill="none" stroke="#334155" strokeWidth="2" className="w-6 h-6"><path d="M21 15v4a2 2 0 0 1-2-2H5a2 2 0 0 1-2-2v-4"></path><polyline points="17 8 12 3 7 8"></polyline><line x1="12" y1="3" x2="12" y2="15"></line></svg></div>
                    )}
                    <p className="text-[#0F172A] font-medium mb-1.5">{fileName || (isDragging ? 'Drop to upload' : 'Click to browse')}</p>
                    <p className="text-[#94A3B8] text-sm">Supports PDF up to 10MB</p>
                  </label>
                </div>

                {/* Link Section */}
                <div className="mb-8">
                  <h2 className="text-[#1E293B] font-medium text-[1.05rem] mb-4">Or paste Google Drive link</h2>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                      <svg viewBox="0 0 24 24" fill="none" stroke="#94A3B8" strokeWidth="2" className="w-5 h-5"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"></path><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"></path></svg>
                    </div>
                    <input
                        type="text"
                        value={linkInput}
                        ...
                        className="w-full pl-11 pr-4 py-3.5 border border-[#E2E8F0] rounded-xl text-[#334155] focus:ring-2 focus:ring-[#3B4569]/20 transition-all bg-white"
                    />
                  </div>
                </div>
                ...
                <button
                    ...
                    className="w-full bg-[#3B4569] hover:bg-[#2D3552] text-white font-semibold py-4 rounded-xl transition-colors shadow-sm disabled:opacity-70"
                >
                  Submit for Review
                </button>
              </div>
              <p className="text-slate-400 text-sm">Your credentials will be verified by our AI within 24 hours</p>
            </div>
        )}
      </div>
  );
